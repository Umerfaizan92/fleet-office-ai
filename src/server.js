...
app.get('/api/product-guide/status',productGuideLimiter,(req,res)=>{const status=aiProviderStatus(),geminiReady=Boolean(geminiApiKey()),openAiReady=Boolean(openAiSpeechConfig());res.json({ok:true,ai:{configured:status.configured,provider:status.provider,model:status.model},free_ai_mode:freeAiModeEnabled(),voice_server_ready:geminiReady||openAiReady,transcription_server_ready:geminiReady||openAiReady,voice_provider:geminiReady&&freeAiModeEnabled()?'gemini-free':openAiReady?'openai':geminiReady?'gemini':null})});
