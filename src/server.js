...
async function sendSmsVerification(to,code,businessName){
  const from=env.TELNYX_FROM_NUMBER||env.TELNYX_PHONE_NUMBER;
  if(!env.TELNYX_API_KEY||!from)return {sent:false,reason:'telnyx_not_configured'};
...
async function deliverRegistrationCodes(row,emailCode,smsCode){
  ...
  if(!verificationTestMode&&(!emailResult.sent||!smsResult.sent)){const missing=[!emailResult.sent?'email':'',!smsResult.sent?'SMS':''].filter(Boolean).join(' and ');const err=new Error(`${missing} verification delivery is not configured or failed. Account creation was stopped securely.`);err.statusCode=503;throw err;}
  return {email:emailResult,sms:smsResult,...(verificationTestMode?{test_codes:{email:emailCode,sms:smsCode}}:{})};
}
...
app.post('/api/saas/registration/start',verificationLimiter,async(req,res)=>{
  ...
  if(db.prepare(`SELECT id FROM users WHERE email=? OR phone=?`).get(email,phone))return res.status(409).json({ok:false,error:'An account already uses this email address or mobile number.'});
  if(db.prepare(`SELECT id FROM organisations WHERE business_identifier_type=? AND business_identifier=?`).get(identifierType,identifier))return res.status(409).json({ok:false,error:'A workspace already exists for this verified business identifier.'});
  ...
  try{
    ...
    db.prepare(`INSERT INTO pending_registrations (id,business_name,legal_name,abn,business_identifier_type,business_identifier,abn_status,abn_payload_json,state,postcode,full_name,email,phone,password_hash,password_salt,plan_id,terms_version,referral_code,email_code_hash,sms_code_hash,expires_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id,parsed.data.business_name,business.legal_name||parsed.data.business_name,business.abn||'',identifierType,identifier,business.status,JSON.stringify({source:business.source,state:business.state,postcode:business.postcode,business_names:business.business_names||[],identifier_type:identifierType,identifier}),parsed.data.state,parsed.data.postcode,parsed.data.full_name,email,phone,passwordHash(parsed.data.password,salt),salt,parsed.data.plan_id,parsed.data.terms_version,referralCode||null,otpHash(id,emailCode),otpHash(id,smsCode),expires.toISOString(),now.toISOString(),now.toISOString());
    let delivery;try{delivery=await deliverRegistrationCodes(row,emailCode,smsCode);}catch(err){db.prepare(`DELETE FROM pending_registrations WHERE id=?`).run(id);throw err;}
    res.status(202).json({ok:true,pending_id:id,expires_at:expires.toISOString(),email:maskEmail(email),phone:maskPhone(phone),business:{identifier_type:identifierType,identifier,status:business.status,legal_name:business.legal_name||parsed.data.business_name,abn:business.abn||'',source:business.source},delivery,test_mode:verificationTestMode,message:'Business identity verified. Enter the security codes sent to your email and Australian mobile.'});
  }catch(err){res.status(err.statusCode||400).json({ok:false,error:err.message});}
});
...
app.post('/api/saas/registration/resend',verificationLimiter,async(req,res)=>{
  ...
  try{const delivery=parsed.data.channel==='email'?await sendSaasVerificationEmail(env,{to:row.email,code,businessName:row.business_name}):await sendSmsVerification(row.phone,code,row.business_name);if(!delivery.sent&&!verificationTestMode)return res.status(503).json({ok:false,error:`${parsed.data.channel==='email'?'Email':'SMS'} verification is not configured.`});res.json({ok:true,channel:parsed.data.channel,expires_at:expires.toISOString(),...(verificationTestMode?{test_code:code}:{}),message:'A new verification code has been issued.'});}catch(err){res.status(503).json({ok:false,error:err.message});}
});
...
app.get('/api/saas/onboarding',requireSaasUser,(req,res)=>{const profile=db.prepare(`SELECT * FROM onboarding_profiles WHERE organisation_id=?`).get(req.saas.organisation_id);res.json({ok:true,profile:{...profile,services:safeJson(profile?.services,[]),custom_sections:safeJson(profile?.custom_sections_json,[])}})});
app.put('/api/saas/onboarding',requireSaasUser,(req,res)=>{const parsed=z.object({business_type:z.string().trim().min(2).max(150),business_structure:z.enum(['sole_trader','company','partnership','trust','not_for_profit','other']).default('sole_trader'),team_mode:z.enum(['solo','team']).default('solo'),phone:z.string().max(50).optional(),website:z.string().max(500).optional(),service_area:z.string().max(1000).optional(),services:z.array(z.string().max(200)).max(100),custom_sections:z.array(z.string().trim().min(1).max(100)).max(30).default([]),ai_setup_mode:z.enum(['assist','manual','ai_first']).default('assist'),brand_voice:z.string().max(2000).optional(),approval_mode:z.enum(['everything','external_actions','custom']),ai_instructions:z.string().max(10000).optional(),complete:z.boolean().default(false)}).safeParse(req.body);if(!parsed.success)return res.status(400).json({ok:false,error:'Check the onboarding information.'});const now=new Date().toISOString();db.prepare(`UPDATE onboarding_profiles SET business_type=?,business_structure=?,team_mode=?,phone=?,website=?,service_area=?,services=?,custom_sections_json=?,ai_setup_mode=?,brand_voice=?,approval_mode=?,ai_instructions=?,completed_at=?,updated_at=? WHERE organisation_id=?`).run(parsed.data.business_type,parsed.data.business_structure,parsed.data.team_mode,parsed.data.phone||null,parsed.data.website||null,parsed.data.service_area||null,JSON.stringify(parsed.data.services),JSON.stringify(parsed.data.custom_sections),parsed.data.ai_setup_mode,parsed.data.brand_voice||null,parsed.data.approval_mode,parsed.data.ai_instructions||null,parsed.data.complete?now:null,now,req.saas.organisation_id);saasAudit(req,'onboarding.updated','organisation',req.saas.organisation_id,{business_structure:parsed.data.business_structure,team_mode:parsed.data.team_mode,ai_setup_mode:parsed.data.ai_setup_mode});res.json({ok:true})});
