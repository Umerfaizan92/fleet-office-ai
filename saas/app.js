...
(function installCopilot(){
  ...
  if(R){let listenIndex=0,finalTranscript='';const r=new R();r.interimResults=true;r.continuous=false;r.maxAlternatives=3;
    const preferredLocale=()=>{const typed=languageOf(text.value);if(typed&&typed!=='en')return localeMap[typed]||'en-AU';const nav=String(navigator.language||'').toLowerCase();return recognitionLocales.find(x=>x.toLowerCase()===nav)||recognitionLocales.find(x=>x.toLowerCase().split('-')[0]===nav.split('-')[0])||recognitionLocales[listenIndex]||'en-AU'};
    r.onstart=()=>{mic.classList.add('listening');voiceStatus.textContent=`Listening · ${r.lang}. Speak naturally…`};
    r.onresult=e=>{let q='';for(let i=0;i<e.results.length;i++)q+=e.results[i][0].transcript+' ';finalTranscript=q.trim();text.value=finalTranscript;const detected=languageOf(finalTranscript);if(detected&&localeMap[detected])voiceStatus.textContent=`Heard ${detected.toUpperCase()} · preparing reply…`;if(e.results[e.results.length-1].isFinal&&finalTranscript)setTimeout(()=>reply(finalTranscript),180)};
    r.onend=()=>mic.classList.remove('listening');
    r.onerror=e=>{mic.classList.remove('listening');if(e.error==='language-not-supported'||e.error==='no-speech'){listenIndex=(listenIndex+1)%recognitionLocales.length;voiceStatus.textContent='I did not catch that language clearly. Tap the mic and try again.'}else voiceStatus.textContent=e.error==='not-allowed'?'Microphone permission is blocked.':'Voice recognition could not start.'};
    mic.onclick=()=>{finalTranscript='';r.lang=preferredLocale();try{r.start()}catch{}}
  }else{mic.title='Voice input is not supported in this browser.';mic.onclick=()=>note('Voice input is not supported in this browser. Chrome or Edge is recommended.',true)}
})();
