(() => {
  ...
  if(page==='verify'){
    ...
    }form.onsubmit=async e=>{e.preventDefault();show('');const b=objectFrom(form);b.pending_id=state.pending_id;button.disabled=true;button.textContent='Verifying…';try{await api('/api/saas/registration/verify',{method:'POST',body:JSON.stringify(b)});sessionStorage.removeItem('gds-pending-registration');sessionStorage.setItem('gds-new-email',state.raw_email||'');sessionStorage.setItem('gds-first-login-tour','1');location.href='sign-in.html?created=1'}catch(err){if(err.payload?.code==='ACCOUNT_EXISTS'||err.payload?.code==='BUSINESS_EXISTS'){show((err.message||'This workspace already exists.')+' Redirecting to sign in…',true);sessionStorage.removeItem('gds-pending-registration');sessionStorage.setItem('gds-new-email',state.raw_email||'');setTimeout(()=>{location.href=err.payload.sign_in_url||'sign-in.html'},1800);return;}show(err.message,true);button.disabled=false;button.textContent='Complete secure verification →'}};
