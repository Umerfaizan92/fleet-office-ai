# Deploy — Super Pro AI Office Manager 18.0.0

## 1. Render source

Deploy the repository's `main` branch. The Node runtime must be Node 20 or newer.
...
Do not rely on ephemeral filesystem storage for production customer data. Configure persistent storage or the intended managed datastore before accepting production customers.
