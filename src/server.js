...
app.post('/api/saas/registration/start',registrationStartLimiter,async(req,res)=>{
  ...
  const existingBusiness=db.prepare(`SELECT id,name FROM organisations WHERE business_identifier_type=? AND business_identifier=?`).get(identifierType,identifier)
    ||db.prepare(`SELECT id,name FROM organisations WHERE lower(trim(name))=lower(trim(?))`).get(parsed.data.business_name);
...
app.post('/api/saas/registration/verify',otpVerifyLimiter,async(req,res)=>{
  ...
  if(configuredValue(env.TELNYX_VERIFY_PROFILE_ID)){
    ...
    smsOk=Boolean(verified.ok);
    const rawSmsReason=String(verified.reason||verified.response_code||'').trim();
    smsReason=/^(accepted|ok|verified|success)$/i.test(rawSmsReason)?'':rawSmsReason;
  }else{
  ...
  const duplicateBusiness=db.prepare(`SELECT id,name FROM organisations WHERE business_identifier_type=? AND business_identifier=?`).get(row.business_identifier_type,row.business_identifier)
    ||db.prepare(`SELECT id,name FROM organisations WHERE lower(trim(name))=lower(trim(?))`).get(row.business_name);
